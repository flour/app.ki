using System.Runtime.Serialization;

namespace AppKi.Commons.Models;

[DataContract]
public class Result : IResult
{
    [DataMember(Order = 1)] public bool Success { get; set; }
    [DataMember(Order = 2)] public string Message { get; set; }
    [DataMember(Order = 3)] public int StatusCode { get; set; }
    [DataMember(Order = 4)] public int ErrorCode { get; set; }

    public static Result Ok(string message = null)
    {
        return New(message, 200, true);
    }

    public static Result Created(string message = null)
    {
        return New(message, 201, true);
    }

    public static Result Updated(string message = null)
    {
        return New(message, 204, true);
    }

    public static Result Bad(string message, int? errorCode = null)
    {
        return New(message, 400, errorCode: errorCode);
    }

    public static Result UnAuthorized(string message, int? errorCode = null)
    {
        return New(message, 401, errorCode: errorCode);
    }

    public static Result Forbidden(string message, int? errorCode = null)
    {
        return New(message, 403, errorCode: errorCode);
    }

    public static Result NotFound(string message, int? errorCode = null)
    {
        return New(message, 404, errorCode: errorCode);
    }

    public static Result Failed(string message, int code = 500, int? errorCode = null)
    {
        return New(message, code, errorCode: errorCode);
    }

    public static Result Failed(IResult result)
    {
        return New(result.Message, result.StatusCode, errorCode: result.ErrorCode);
    }

    public static Result Internal(string message, int? errorCode = null)
    {
        return New(message, 500, errorCode: errorCode);
    }

    public static Result Code(string message, int code, int? errorCode = null)
    {
        return New(message, code, errorCode: errorCode);
    }

    private static Result New(
        string message,
        int code = 500,
        bool success = false,
        int? errorCode = null)
    {
        return new()
        {
            Message = message,
            StatusCode = code, 
            Success = success, 
            ErrorCode = errorCode ?? (success ? default : IResult.DefaultErrorCode)
        };
    }
}

[DataContract]
public class Result<T> : IResult<T>
{
    [DataMember(Order = 3)] public T Data { get; set; }
    [DataMember(Order = 1)] public bool Success { get; set; }
    [DataMember(Order = 2)] public string Message { get; set; }
    [DataMember(Order = 4)] public int StatusCode { get; set; }
    [DataMember(Order = 5)] public int ErrorCode { get; set; }

    public static Result<T> Ok(T data, string message = null)
    {
        return New(message, 200, data, true);
    }

    public static Result<T> Created(T data, string message = null)
    {
        return New(message, 201, data, true);
    }

    public static Result<T> Updated(T data = default, string message = null)
    {
        return New(message, 204, data, true);
    }

    public static Result<T> Bad(string message, int? errorCode = null)
    {
        return New(message, 400, errorCode: errorCode);
    }

    public static Result<T> UnAuthorized(string message, int? errorCode = null)
    {
        return New(message, 401, errorCode: errorCode);
    }

    public static Result<T> Forbidden(string message, int? errorCode = null)
    {
        return New(message, 403, errorCode: errorCode);
    }

    public static Result<T> NotFound(string message, int? errorCode = null)
    {
        return New(message, 404, errorCode: errorCode);
    }

    public static Result<T> Failed(string message, int code = 500, T data = default, int? errorCode = null)
    {
        return New(message, code, data, errorCode: errorCode);
    }

    public static Result<T> Failed(IResult result)
    {
        return New(result.Message, result.StatusCode, errorCode: result.ErrorCode);
    }

    public static Result<T> Failed(Result<T> result)
    {
        return New(result.Message, result.StatusCode, result.Data, errorCode: result.ErrorCode);
    }

    public static Result<T> Internal(string message, int? errorCode = null)
    {
        return New(message, errorCode: errorCode);
    }

    private static Result<T> New(
        string message,
        int code = 500,
        T data = default,
        bool success = false,
        int? errorCode = null)
    {
        return new()
        {
            Message = message,
            StatusCode = code,
            Success = success,
            Data = data,
            ErrorCode = errorCode ?? (success ? default : IResult.DefaultErrorCode),
        };
    }
}